...
public class ViewEventAction extends CalendarAbstractAction
{
	...
	public String execute() throws Exception 
	{
		try
		{
			...
			if (this.versionLanguageId == null && requestLanguageId != null && !requestLanguageId.equals("")) {
				this.versionLanguageId = new Long(requestLanguageId);
			}
			...
			if(this.eventId != null)
			{
				...
				while(eventVersionsIterator.hasNext())
				{
					...
					// If we haven't set a language yet, use the language of this event version (the first)
					if(this.versionLanguageId == null)
					{
						this.versionLanguageId = currentEventVersion.getVersionLanguageId();
					}
				...
				// If we still don't have a version language set, use the first from the available languages.
				...
				System.out.println("test rasmus: " + copyDescriptionFieldsForNewLanguageVersion(this.calendarId));
