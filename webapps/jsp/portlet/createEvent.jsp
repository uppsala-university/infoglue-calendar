    	...
    	if (lastRegistrationTime != "" && !validateTime(lastRegistrationTime))
    	{
          ...
          if (!document.inputForm.agree.checked) {
              document.inputForm.agree.focus();
               alert("Du m�ste fylla i kryssrutan f�r att g� vidare");
...
<fieldset>
	...
	<c:set var="tabCounter" value="25"/>
		<ww:iterator value="calendar.eventType.categoryAttributes" status="rowstatus">
			...
			<calendar:selectField label="top.name" name="${categoryAttributeName}" multiple="true" value="top.category.children" selectedValues="getCategoryAttributeValues(top.id)" tabIndex="${tabCounter}" cssClass="listBox" required="true"/>
